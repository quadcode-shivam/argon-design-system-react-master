...
const createLeave = async (data) => {
  ...
  try {
    ...
    const leaveData = {
      ...
      half_day_full_day: data.halfDayFullDay, // Whether it’s a full day or half day leave
    };
...
const removeLeave = async (data) => {
  try {
    const response = await api.post("/remove-leave",{"leave_id":data}); // Adjust if API expects a body or params
    console.log("Leaves data deled successfully:", response.data);
    return response.data;
  } catch (error) {
    console.error("Error fetching leaves:", error.response || error);
    throw new Error("Failed to delete leaves data. Please try again later.");
  }
};
